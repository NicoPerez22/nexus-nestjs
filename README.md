# NEXUS HQ — API NestJS + MySQL

Backend de las páginas de NEXUS HQ Angular 21. NestJS 11, TypeORM 0.3 y MySQL 8, pensado para desplegarse contra una base MySQL de Hostinger. Cada página tiene su controller, service y module dentro de `src/feature`. Las versiones exactas quedan fijadas en `package-lock.json`.

## Inicio local

Requisitos: Node.js 22 o 24, npm y Docker Compose (o un servidor MySQL 8 existente, por ejemplo el de Hostinger).
...
Docker crea la base `nexus_hq` y mantiene sus datos en un volumen. Las credenciales de MySQL del Compose son exclusivamente para desarrollo local. Para producción, creá una base vacía en tu servidor MySQL de Hostinger y configurá `MYSQLHOST`, `MYSQLPORT`, `MYSQLUSER`, `MYSQLPASSWORD` y `MYSQLDATABASE`. No se debe exponer MySQL a Internet sin restricciones (usá la lista de hosts permitidos de Hostinger).
...
- `npm run db:seed`: insertar organización, administrador, equipos, áreas, eventos, tareas, categorías financieras y un prospecto de scouting de ejemplo.
- `npm test`: compilar y ejecutar pruebas HTTP de integración con TypeORM y pg-mem.
- `docker compose stop`: detener MySQL conservando los datos.
...
- `src/feature/teams`: equipos, CRUD y edición transaccional de roster.
...
- `src/feature/finance`: categorías, ingresos/egresos, detalle y datos para gráficos.
- `src/feature/scouting`: prospectos, características y observaciones.
...
- `database/001-schema.sql`: SQL MySQL completo como referencia o instalación manual.
- `database/002-finanzas-scouting.sql`: SQL para agregar finanzas y scouting sobre una base ya creada.
...
Tablas: `users`, `sessions`, `organization`, `teams`, `players`, `areas`, `events`, `tasks`, `roles`, `finance_categories`, `finance_movements`, `scout_players`, `scout_traits`, `scout_observations`; además TypeORM crea su tabla `migrations`.
...
La migración es la ruta recomendada. El archivo SQL refleja el mismo esquema. **No ejecutar el SQL manual y luego la migración sobre esa misma base**, porque intentaría crear tablas existentes. El seed funciona con cualquiera de las dos rutas. Los IDs (`uuid` a nivel de aplicación) se guardan como `varchar(36)`, porque MySQL no tiene un tipo `uuid` nativo.
...
Roles: `admin` y `manager` pueden leer y modificar; `viewer` solo leer y cerrar su sesión. El seed crea un admin y los roles `admin`, `manager` y `viewer`. `POST /auth/users` permite a un admin o manager crear usuarios; no hay registro público. El rol se resuelve por `users.role_id` → `roles`.
...
Compilación TypeScript y cinco pruebas de integración HTTP aprobadas: autenticación/perfil/organización, métricas y roster, CRUD de eventos, CRUD y estado de tareas, seed/roles/revocación/expiración. Usan pg-mem (emulación PostgreSQL), no un servidor MySQL real; la migración se escribió en SQL compatible con ambos motores para que las pruebas sigan corriendo sobre esta emulación aunque producción use MySQL. El Compose y la migración se entregan para ejecutar la base real; no se ha validado Docker en este entorno. Los tests comprueban operaciones y contratos, pero no sustituyen pruebas de concurrencia ni de códigos de error específicos de MySQL real.
