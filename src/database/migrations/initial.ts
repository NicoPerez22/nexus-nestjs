...
export class Initial1788900000000 implements MigrationInterface {
 ...
 expires_at timestamp NOT NULL
    ...
    await q.query(`CREATE INDEX sessions_expiry_idx ON sessions(expires_at)`);
    await q.query(`CREATE TABLE teams (
 ...
 CONSTRAINT team_win CHECK (win >= 0 AND win <= 100)
    ...
    await q.query(`CREATE TABLE areas (
 ...
 name varchar(40) NOT NULL, role varchar(40) NOT NULL, position integer NOT NULL
    ...
    await q.query(`CREATE INDEX players_team_idx ON players(team_id)`);
 ...
 CONSTRAINT event_type CHECK (type IN ('Entrenamiento','Competencia','Reunión'))
    ...
    await q.query(`CREATE INDEX events_agenda_idx ON events(date,time)`);
    await q.query(`CREATE INDEX events_area_idx ON events(area_id)`);
    await q.query(`CREATE TABLE tasks (
 id varchar(36) PRIMARY KEY, name varchar(100) NOT NULL,
 area_id varchar(36) NOT NULL REFERENCES areas(id) ON DELETE RESTRICT,
 who varchar(60) NOT NULL, due date NOT NULL,
 priority boolean NOT NULL DEFAULT false, done boolean NOT NULL DEFAULT false
)`);
    await q.query(`CREATE INDEX tasks_status_due_idx ON tasks(done,due)`);
    await q.query(`CREATE INDEX tasks_area_idx ON tasks(area_id)`);
    await q.query(`CREATE TABLE finance_categories (
 id varchar(36) PRIMARY KEY, name varchar(100) NOT NULL UNIQUE,
 kind varchar(20) NOT NULL,
 CONSTRAINT finance_category_kind CHECK (kind IN ('ingreso','egreso','ambos'))
)`);
    await q.query(`CREATE TABLE finance_movements (
 ...
 CONSTRAINT finance_amount CHECK (amount > 0)
    ...
    await q.query(`CREATE INDEX finance_movements_date_idx ON finance_movements(date)`);
    await q.query(`CREATE INDEX finance_movements_category_idx ON finance_movements(category_id)`);
    await q.query(`CREATE TABLE scout_players (
 id varchar(36) PRIMARY KEY, name varchar(100) NOT NULL,
 ...
 CONSTRAINT scout_status CHECK (status IN ('Seguimiento','Contactado','Prueba','Fichado','Descartado'))
    ...
    await q.query(`CREATE TABLE scout_traits (
 ...
 CONSTRAINT scout_rating CHECK (rating >= 1 AND rating <= 10)
    ...
    await q.query(`CREATE INDEX scout_traits_player_idx ON scout_traits(player_id)`);
    await q.query(`CREATE TABLE scout_observations (
 id varchar(36) PRIMARY KEY,
 player_id varchar(36) NOT NULL REFERENCES scout_players(id) ON DELETE CASCADE,
 date date NOT NULL, author varchar(60) NOT NULL,
 title varchar(100) NOT NULL, content varchar(2000) NOT NULL
)`);
    await q.query(`CREATE INDEX scout_observations_player_idx ON scout_observations(player_id)`);
  }
  async down(q: QueryRunner) {
    await q.query(
      "DROP TABLE scout_observations, scout_traits, scout_players, finance_movements, finance_categories, tasks, events, players, areas, teams, sessions, users, roles, organization",
    );
  }
}
